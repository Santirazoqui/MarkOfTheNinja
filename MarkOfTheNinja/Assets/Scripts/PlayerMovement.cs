...
public class ExtendedPlayerController : MonoBehaviour {
        ...
        #region Extended script variables

        // Extended script bonuses
        ...
        private void GatherInput() {
            Input = new FrameInput {
                ...
                JumpUp = UnityEngine.Input.GetButtonUp("Jump")|| UnityEngine.Input.GetKeyUp(KeyCode.C),
                X = UnityEngine.Input.GetAxisRaw("Horizontal"),
                Y = UnityEngine.Input.GetAxisRaw("Vertical"),
            };
            _dashPressed = UnityEngine.Input.GetKeyDown(KeyCode.X);
            if (Input.JumpDown) {
        ...
        private void CalculateJump() {
            DoubleJumpingThisFrame = false;
            if (Input.JumpDown && CanDoubleJump) {
                _currentVerticalSpeed = _jumpHeight;
                _doubleJumpUsable = false;
                _endedJumpEarly = false;
                DoubleJumpingThisFrame = true;
            }

            // Jump if: grounded or within coyote threshold || sufficient jump buffer
            else if (Input.JumpDown && CanUseCoyote || HasBufferedJump) {
                _currentVerticalSpeed = _jumpHeight;
                _endedJumpEarly = false;
                _coyoteUsable = false;
        ...
        [SerializeField] private float _dashLength = 0.25f;
        private float _startedDashing;
        private bool _canDash;
        private Vector2 _dashVel;
        private Vector2 _startedDashPoint;
        private float _safestDashDistance;
        void CalculateDash() {
            if (_dashPressed && _canDash) {
                var vel = new Vector2(Input.X, _colDown && Input.Y < 0 ? 0 : Input.Y);
                if(vel == Vector2.zero) return;
                _dashVel = vel * _dashPower;
                Dashing = true;
                _canDash = false;
                _startedDashing = Time.time;
                _startedDashPoint = transform.position;
                
                // Dash is very quick, so we'll need to do some hacky safety checks to prevent collisions
                // Cast your bounds in the direction of the dash to find the first collision, then remove your extents from the distance
                var dashHit = Physics2D.BoxCast(transform.position, _characterBounds.size, 0, vel, 10, _groundLayer);
                _safestDashDistance = dashHit.collider != null ? dashHit.distance - _characterBounds.extents.x : Single.MaxValue;
            }
            ...
            if (Dashing) {
                ...
                if (_startedDashing + _dashLength < Time.time || Vector2.Distance(_startedDashPoint,transform.position) >= _safestDashDistance) {
                    Dashing = false;
                    _currentVerticalSpeed = 0;
                    _currentHorizontalSpeed *= 0.25f;
                    if (_colDown) _canDash = true;
                }
            }
        }
